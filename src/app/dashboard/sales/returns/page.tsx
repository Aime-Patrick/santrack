...
import { ArrowUpDown, RotateCcw, Clock, CheckCircle, XCircle, MoreHorizontal } from "lucide-react";
...
import { useReturns, useApproveReturn, useRejectReturn, useRefundReturn } from "@/hooks/commerce";
...
export default function ReturnsPage() {
  ...
  const columns: ColumnDef<TableFeatures, SalesReturn>[] = useMemo(() => [
    {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              {ret.status === "PENDING" && (
                <>
                  <DropdownMenuItem onClick={() => approveReturn.mutate({ id: ret.id, data: { refundMethod: "ORIGINAL_PAYMENT" } })}>
                    <CheckCircle className="mr-2 size-4" /> Approve
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => rejectReturn.mutate({ id: ret.id, data: { reason: "Rejected by admin" } })} className="text-destructive">
                    <XCircle className="mr-2 size-4" /> Reject
                  </DropdownMenuItem>
                </>
              )}
              {ret.status === "APPROVED" && (
                <DropdownMenuItem onClick={() => refundReturn.mutate(ret.id)}>
                  <RotateCcw className="mr-2 size-4" /> Process Refund
                </DropdownMenuItem>
              )}
            </DropdownMenuContent>
          </DropdownMenu>
  ...
  ], [approveReturn, rejectReturn, refundReturn]);
