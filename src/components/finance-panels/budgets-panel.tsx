...
import { useBudgets, useCreateBudget, useAccounts, useCostCentres } from "@/hooks/finance";
  ...
  return (
    <div className="space-y-6">
      ...
      <ResourceFormDialog
        ...
        title="Add a budget"
        description="What one account is allowed to spend or expected to earn in a period. Variance reports compare posted journal lines against this."
        ...
        fields={[
          ...
          },
          {
            ...
            required: true,
            half: true,
            hint: "The month this budget covers.",
          },
          {
            name: "amount",
            label: "Amount",
          ...
          },
          {
            ...
            label: "Cost centre",
            kind: "select",
            options: (costCentres ?? []).map((c) => ({
              value: String(c.id),
              label: c.name,
              hint: c.code,
            })),
            emptyMessage: "None defined yet",
            hint: "Leave blank to budget the account as a whole.",
          },
        ]}
        onSubmit={(v) =>
          create.mutate(
            {
              ...
              period: v.period,
              // Sent as a string: money loses cents through a float, so the
              // API takes the digits as typed and parses them as a decimal.
              amount: v.amount.trim(),
              costCentreId: num(v, "costCentreId"),
            },
